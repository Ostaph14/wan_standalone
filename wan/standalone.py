...
class WanVideoGenerator:
	...
	def directly_manage_blocks(self, transformer, blocks_to_keep):
		"""
		...
		# Then move only the blocks we don't need to CPU
		for i in range(blocks_to_keep, total_blocks):
			transformer.blocks[i] = transformer.blocks[i].to(self.offload_device)
	...
	def sample(self, model_info, text_embeds, image_embeds, shift, steps, cfg, seed=None,
		...
		# Apply direct block management
		if model_info["block_swap_args"] is not None:
			blocks_to_keep = model_info["block_swap_args"]["blocks_to_swap"]
			transformer = self.directly_manage_blocks(transformer, blocks_to_keep)
		...
		if scheduler == 'unipc':
			sample_scheduler = FlowUniPCMultistepScheduler(
				...
				shift=shift,
				use_dynamic_shifting=True,)  # Only enable dynamic shifting
			sample_scheduler.set_timesteps(
				steps, device=self.device, shift=shift, mu=shift)
		...
		# Prepare arguments
		base_args = {
			'clip_fea': image_embeds.get('clip_context', None).to(self.device) if image_embeds.get('clip_context',
		...
		if transformer.model_type == "i2v":
			base_args.update({
				'y': [image_embeds["image_embeds"].to(self.device)],
			})

		arg_c = base_args.copy()
		arg_c.update({'context': [text_embeds["prompt_embeds"][0].to(self.device)]})

		arg_null = base_args.copy()
		arg_null.update({'context': [t.to(self.device) for t in text_embeds["negative_prompt_embeds"]]})

...
class WanVideoGenerator:
    ...
    def load_model(self, model_path, base_precision="bf16", load_device="main_device",
        ...
        base_dtype = {
            "fp8_e4m3fn": torch.float8_e4m3fn if hasattr(torch, "float8_e4m3fn") else torch.float32,
            "fp8_e4m3fn_fast": torch.float8_e4m3fn if hasattr(torch, "float8_e4m3fn") else torch.float32,
            "bf16": torch.bfloat16,
            "fp16": torch.float16,
            "fp32": torch.float32
        }[base_precision]
        ...
        if not "torchao" in quantization:
            ...
            if quantization in ["fp8_e4m3fn", "fp8_e4m3fn_fast", "fp8_scaled"] and hasattr(torch, "float8_e4m3fn"):
                dtype = torch.float8_e4m3fn
            elif quantization == "fp8_e5m2" and hasattr(torch, "float8_e5m2"):
            ...
            for name, param in transformer.named_parameters():
                ...
                set_module_tensor_to_device(transformer, name, device=transformer_load_device, dtype=dtype_to_use,
                                            value=sd[name])
    ...
    def encode_image_clip(self, clip, vae, image, num_frames, generation_width, generation_height,
        ...
        # For PyTorch tensor input
        if isinstance(image, torch.Tensor):
            # If image is [B, H, W, C], move to [B, C, H, W]
            if image.shape[-1] == 3:
            ...
            # Convert PIL image to tensor
            resized_image = torch.tensor(np.array(image).astype(np.float32) / 255.0).unsqueeze(0).permute(0, 3, 1, 2)
    ...
    def sample(self, model_info, text_embeds, image_embeds, shift, steps, cfg, seed=None,
        ...
        with torch.autocast(device_type=self.device.split(':')[0], dtype=model_info["dtype"], enabled=True):
            for i, t in enumerate(tqdm(timesteps)):
                ...
                latent = latent.to(self.offload_device)
    ...
    def generate_video(self,
        ...
        # Add this after line 821 in your generate_video function
...
def main():
    ...
    parser.add_argument("--blocks_to_swap", type=int, default=20, help="Number of blocks to swap to CPU to save VRAM")
    ...
    # CRUCIAL VRAM-SAVING PARAMETERS
    # Manually specify paths to match ComfyUI workflow
    ...
    # Generate video with explicit parameters matching ComfyUI
    generator.generate_video(
        model_path=model_path,
        vae_path=vae_path,
        t5_path=t5_path,
        clip_path=clip_path,
        base_precision="fp16",  # Use fp16 for initial loading
        vae_precision="bf16",
        t5_precision="bf16",
        clip_precision="fp16",
        quantization="fp8_e4m3fn",  # CRUCIAL: Use fp8 quantization like ComfyUI
        attention_mode="sdpa",
        blocks_to_swap=args.blocks_to_swap,  # Match ComfyUI's 20 blocks to swap
